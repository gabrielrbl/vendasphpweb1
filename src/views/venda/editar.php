<?php
    ...
    $itensvenda = $itensvenda->findAllByIdVenda($venda->getId());
?>
<!doctype html>
...
<head>
    ...
    <title>Sistema de vendas | Editar venda</title>
                ...
                <div class="grid-x grid-padding-x" id="produtos">
                            ...
                            <select name="idcliente" required>
                                <?php
                                    foreach ($clientes->findAll() as $obj) { 
                                        if ($obj->getId() == $venda->getIdcliente()) { ?>
                                            <option selected value="<?= $obj->getId() ?>" disabled><?= $obj->getNome() ?></option><?php
                                        }
                                    }
                                ?>
                            </select>
                    ...
                    <?php
                        foreach ($itensvenda as $itemvenda) { ?>
                                        ...
                                        <option value="" selected disabled>Selecione um produto</option>
                                            <?php
                                                foreach ($produtos->findAll() as $obj) { 
                                                    if ($obj->getId() == $itemvenda->getIdproduto()) { ?>
                                                        <option selected value="<?= $obj->getId() ?>"><?= $obj->getNome() . ' - ' . $obj->getValor() ?></option> <?php
                                                    } else { ?>
                            ...
                            <div class="input medium-2 cell" id="quantidade">
                                <label>Quantidade
                                    <input type="number" value="<?= $itemvenda->getQuantidade() ?>" min="1" name="quantidade[]" required>
                                </label>
                            </div>
                            <div class="input medium-2 cell" id="valor">
                                <label>Valor unidade
                                    <input type="number" value="<?= $itemvenda->getValorunitario() ?>" name="valor" disabled>
                                </label>
                            </div>
                        <?php } ?>
                    </div>

                    <input type="submit" class="button" value="Atualizar venda">
                </div>
            </div>
        </form>
    </div>
